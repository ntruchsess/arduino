...
{
  wireWriteByte(COPY_SCRATCHPAD);
  if (!mState)
    {
      for (int i=0; i < 8 && !mState; i++)
        wireWriteByte(rom[i]);
    }
}
...
{
  return wireSearchInternal(SEARCH, newAddr);
}

uint8_t DS2482::wireSearchAlarms(uint8_t *newAddr)
{
  return wireSearchInternal(SEARCH_ALARMS, newAddr);
}

uint8_t DS2482::wireSearchInternal(uint8_t command, uint8_t *newAddr)
{
  uint8_t i;
  uint8_t direction;
  uint8_t last_zero=0;
